    ...
    <h1 style ="text-align: center;">Tabla de datos de Inmigrantes</h1>
        ...
        {#each entries as entry}
            <tr>
                ...
                <td><a href="api/v1/immigrants/{entry.country}/{entry.year}">{entry.country}</a></td>
                <td>{entry.year}</td>
                <td>{entry.men}</td>
                <td>{entry.women}</td>
                <td>{entry.percentages}</td>
                <td><Button outline color="danger" on:click="{deleteStat(entry.country, entry.year)}">Borrar</Button></td>
                <td><a href="#/immigrants/{entry.country}/{entry.year}"><Button outline color="primary">Editar</Button></a></td>
            </tr>
